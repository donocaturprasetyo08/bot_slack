...
class SlackListClient:
	...
	def __init__(self, token: Optional[str] = None):
		...
		if not self.token:
			raise ValueError("Provide SLACK_USER_TOKEN or SLACK_BOT_TOKEN for Slack Lists access")
	...
	def _api_call(self, method: str, **payload: Any) -> Dict[str, Any]:
		try:
			...
			raise RuntimeError(f"Slack API error calling {method}: {exc.response['error']}") from exc

		if not response.get("ok", False):
			raise RuntimeError(f"Slack API error calling {method}: {response.get('error')}")
		return response

	def get_item(self, list_id: str, item_id: str, *, team_id: Optional[str] = None) -> Dict[str, Any]:
	...
	def list_items(self, list_id: str, *, cursor: Optional[str] = None, limit: Optional[int] = None,
	...
	def iter_items(self, list_id: str, *, team_id: Optional[str] = None, page_size: Optional[int] = None) -> Iterable[Dict[str, Any]]:
		"""Yield every item for ``list_id`` by following cursors."""

		cursor = None
		while True:
			response = self.list_items(list_id, cursor=cursor, limit=page_size, team_id=team_id)
	...
	def delete_item(self, list_id: str, item_id: str, *, team_id: Optional[str] = None) -> bool:
		"""Delete a list item."""
