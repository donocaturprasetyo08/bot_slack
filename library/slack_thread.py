...
class SlackThreadClient:
    ...
    def __init__(self, token: Optional[str] = None):
        self.token = token or config.slack_bot_token or config.slack_user_token
        if not self.token:
        ...
        *,
        ...
        while True:
            try:
                ...
                raise RuntimeError(
                    f"Slack API error fetching thread: {exc.response['error']}"
                ) from exc
    ...
    def reply(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                f"Slack API error posting reply: {exc.response['error']}"
            ) from exc
