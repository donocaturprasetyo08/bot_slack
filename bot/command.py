...
from library import slack_thread, sheets, llm, slacklist
...
from bot.helpers import validate_and_extract_command, parse_slack_permalink, get_sheet_name, prepare_thread_content, parse_analysis_response
...
def handle_pqf(channel: str, thread_ts: str, thread_data: dict = None, slack_client = None, from_value: str = "Internal", product: str = "AgentLabs") -> str:
    ...
    try:
        ...
        existing_links = [l.split('&cid=')[0] if l else l for l in _get_sheets_client().get_all_links(sheet_name)]
        ...
        if slack_client and thread_data:
            ...
            if parent_ts:
                ...
                if 1 <= bulan <= 3:
                    ...
                    quarter = 'Q4'
                forward_message(slack_client, thread_data, quarter, tahun, week_num, dt, success=True)
...
def handle_resolution(channel: str, thread_ts: str, thread_data: dict = None, slack_client = None, resolution: str = "Unknown") -> str:
    ...
    try:
        ...
        for sheet_name in sheets_client.get_available_sheets():
            links = [l.split('&cid=')[0] if l else l for l in sheets_client.get_all_links(sheet_name)]
            if permalink in links:
                ...
                sheets_client.append_rows(f"{sheet_name}!J:J", [[resolution]])  # Assuming column J is resolution
                found = True
                break
...
def handle_resolve(channel: str, thread_ts: str, thread_data: dict = None, slack_client = None) -> str:
...
def handle_ticket(channel: str, thread_ts: str, thread_data: dict = None, slack_client = None, title: str = "New Ticket") -> str:
    ...
    try:
        ...
        existing_codes = sheets_client.get_all_codes(sheet_name)  # Assume method exists or simplify
...
def handle_confirm_bug(channel: str, thread_ts: str, thread_data: dict = None, slack_client = None, from_value: str = "Internal") -> str:
...
def handle_command(command: str, channel: str = "", thread_ts: str = "", thread_data: dict = None, slack_client = None, text: str = "", **kwargs) -> str:
    ...
    if command == "pqf":
        ...
        if error:
            return error
        return handle_pqf(channel, thread_ts, thread_data, slack_client, from_value, product)
    elif command == "resolution":
        resolution = kwargs.get('resolution', 'Unknown')
        return handle_resolution(channel, thread_ts, thread_data, slack_client, resolution)
        ...
        if not from_value:
            from_value = "Internal"
        return handle_confirm_bug(channel, thread_ts, thread_data, slack_client, from_value)
    else:
