...
from bot.helpers import prepare_thread_content, parse_analysis_response
...
async def handle_parent_message(event, channel, ts):
    ...
    quarter = 'Q1' if 1 <= bulan <= 3 else 'Q2' if 4 <= bulan <= 6 else 'Q3' if 7 <= bulan <= 9 else 'Q4'
    ...
    if product_sheet != 'Unknown':
        ...
        existing_links = [l.split('&cid=')[0] if l else l for l in sheets_client.get_all_links(sheet_name)]
        ...
        forward_message(slack_bot.client, thread_data, quarter, tahun, week_num, dt, success=True)
    else:
        # Forward as not recorded
        forward_message(slack_bot.client, thread_data, quarter, tahun, week_num, dt, success=False)
...
async def handle_thread_reply(event, channel, ts, thread_ts, text):
    ...
    if command:
        if command == 'pqf' and channel not in FORWARD_CHANNEL_ID:
            slack_bot.send_message(channel, "PQF command not allowed in this channel.", thread_ts=ts)
            return
        result = handle_command(command, channel, thread_ts, thread_data, slack_bot.client, text)
